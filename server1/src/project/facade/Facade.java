...
public class Facade {
    ...
    public ServiceMessage execute(Message message) throws IOException, ClassNotFoundException, SQLException, InvalidKeySpecException, NoSuchAlgorithmException, ParseException, JOSEException {
        ...
        System.out.println("############################################################################");
        System.out.println("service Name = "+executed_service);
        ...
        if(!before_aspects_for_this_service.isEmpty() && message.getParameters()!=null ){
            ...
            for (String s:before_aspects_for_this_service) {
                ...
                message.setParameters((Map) aspects.get(s).before(message.getParameters()));
        }
        }
        ...
        if(service_result.get_result().containsKey("error")) {

            HashMap<String, String> result = (HashMap<String, String>) service_result.get_result();

            result.put("encrypt","1");
